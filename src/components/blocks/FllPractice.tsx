...
export default function FllPractice({lessonId}: Props): React.JSX.Element {
  ...
  const hostRef = useRef<HTMLDivElement>(null); const shellRef = useRef<HTMLDivElement>(null); const workspaceRef = useRef<Blockly.WorkspaceSvg | null>(null); const importRef = useRef<HTMLInputElement>(null);
  const [result, setResult] = useState<FllRunResult | null>(null); const [stepIndex, setStepIndex] = useState(-1); const [playing, setPlaying] = useState(false); const [message, setMessage] = useState<string | null>(null); const [saving, setSaving] = useState(false);
  ...
  const importWorkspace = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value = ''; if (!file || !workspaceRef.current) return; try { const parsed = JSON.parse(await file.text()) as Partial<WorkspaceFile>; if (parsed.format !== 'telemark-fll-block-workspace' || parsed.version !== 1 || !parsed.workspace) throw new Error('That file is not a Telemark FLL workspace.'); if (workspaceRef.current.getAllBlocks(false).length && !window.confirm('Replace the blocks currently in this workspace?')) return; workspaceRef.current.clear(); Blockly.serialization.workspaces.load(parsed.workspace, workspaceRef.current); saveWorkspace(lessonId, workspaceRef.current); setResult(null); setStepIndex(-1); setMessage(parsed.lessonId === lessonId ? 'Workspace imported.' : 'Workspace imported from another FLL lesson.'); } catch (reason) { setMessage(reason instanceof Error ? reason.message : 'Could not import that workspace.'); } };
