...
**3. Mechanical Fail-Safe (Deadzones):** Explain the hardware-focused reason for implementing a "Deadzone" logic check (e.g., `if (Math.abs(input) < 0.1) input = 0;`) in a competition-ready OpMode.
...
<ScoredQuiz
  ...
  questions={SOFTWARE_QUESTIONS['unit-04']}
/>
