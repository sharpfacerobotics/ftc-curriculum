...
public class LimelightFusionDemo extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("Fused X", "%.2f in", estimate.getX());
            telemetry.addData("Fused Y", "%.2f in", estimate.getY());
            telemetry.update();
        }
    ...
    private void growPredictionUncertainty() {
        Pose predicted = follower.getPose();
        double distance = Math.hypot(
            predicted.getX() - previousPrediction.getX(),
            predicted.getY() - previousPrediction.getY()
        );
        double turn = Math.abs(AngleUnit.normalizeRadians(
            predicted.getHeading() - previousPrediction.getHeading()
        ));
    ...
    private void applyVisionMeasurement() {
        ...
        Pose predicted = follower.getPose();
        double innovationX = measuredX - predicted.getX();
        double innovationY = measuredY - predicted.getY();
        double innovationHeading = AngleUnit.normalizeRadians(
            measuredHeading - predicted.getHeading()
        );
        ...
        Pose fused = new Pose(
            predicted.getX() + gainX * innovationX,
            predicted.getY() + gainY * innovationY,
            AngleUnit.normalizeRadians(
                predicted.getHeading() + gainHeading * innovationHeading
            )
        );
