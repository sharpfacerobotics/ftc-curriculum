...
A common pattern is to set `BRAKE` on all drive motors in `init()`, then send `setPower(0.0)` as the last command of every autonomous step to guarantee a clean stop before the next movement begins.
