...
Use the simulator below to practice wiring a reusable mechanism class into a TeleOp. Hover over the open-file tabs and select **+**, then create `LinearSlide.java`. The simulator always starts Java files with:

```java
package org.firstinspires.ftc.teamcode;
```

Move the starter's `LinearSlide` class into that file, make it `public`, and keep the `DcMotor`, `DigitalChannel`, `AnalogInput`, and `HardwareMap` imports with it. Imports apply to one source file only: importing `DcMotor` in the OpMode does not make it available in `LinearSlide.java`. `LinearSlide` itself needs no import in the OpMode because both classes use the same package. These are the [Java package and import rules](https://docs.oracle.com/javase/specs/jls/se24/html/jls-7.html).
