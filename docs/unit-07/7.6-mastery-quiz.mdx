...
The `hardwareMap` looks up devices in the active Robot Controller configuration. A normal `new DcMotor()` call would not know which configured hub and port to use, and FTC hardware classes are supplied by the SDK rather than constructed directly by team code.
...
<ScoredQuiz
  ...
  questions={SOFTWARE_QUESTIONS['unit-07']}
/>
