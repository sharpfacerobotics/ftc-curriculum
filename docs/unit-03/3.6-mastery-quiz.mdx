...
**4. Configuration Management:** Why is it considered a useful practice to store hardware names (like `"left_drive"`) in a `String` variable at the class level instead of passing a literal string directly into `hardwareMap.get()`?
...
<ScoredQuiz
  ...
  questions={SOFTWARE_QUESTIONS['unit-03']}
/>
