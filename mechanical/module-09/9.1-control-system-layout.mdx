...
The control system is the boundary between the mechanical and software halves of the team, and it is where miscommunication becomes a mid-match failure. A motor plugged into a port that the code does not expect looks like a software bug and is not one.
...
When something does not respond, walk the chain from the battery outward. The failure is almost always at a connection rather than inside a component.
...
The single highest value habit in FTC wiring: label every wire at both ends with the name the software uses.
...
Two reasons this matters more than it seems.
...
Use a label maker or write on heat shrink. Do the labeling when the wire is installed, not later, because later does not happen.
...
When the software team asks "which port is the slide on," the answer should be a document, not a person's memory.

---

## Physical Placement

**Keep the Control Hub accessible.** Ports must be reachable without disassembling the robot, and the status lights must be visible. A hub buried under a mechanism is a hub nobody can diagnose at competition.
