...
Lesson 5.2 covered sketches. This lesson covers the container those sketches live in, the ordered list that turns them into geometry, and the datums everything is measured from. Get these three right and the rest of the module is detail.
...
This is the single highest-value habit in the module. A sketch attached to a model face dies when that face is filleted, shelled, or removed. A sketch on a datum plane survives every one of those edits. The cost is one extra plane; the saving is not rebuilding the part in February.
:::
