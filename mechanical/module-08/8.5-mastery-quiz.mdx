...
A failed acquisition does not cost the acquisition time; it costs a full retry, which includes repositioning the robot, reapproaching, and trying again, often several seconds. At a 20% failure rate, one cycle in five carries that penalty, which averages out to a much larger cost per cycle than half a second.
