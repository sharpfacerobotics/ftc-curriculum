...
Linear slides give reach far beyond the robot's frame, which is why almost every high scoring FTC robot has them. They also stack tolerances, accumulate friction, and multiply forces in ways that catch teams out.
...
A larger spool is faster and weaker; a smaller spool is slower and stronger. It behaves exactly like a gear ratio, and it is often the easiest thing to change on an existing slide.
...
Two consequences: the scoring geometry must tolerate the droop, or be measured with the droop included; and the droop loads the stages in a way that increases friction, which is why a slide that moves freely unloaded can stall loaded at full extension.
