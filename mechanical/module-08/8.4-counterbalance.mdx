...
A motor that spends a match holding an arm up is doing no useful work, drawing current, and getting hot. A counterbalance transfers that job to a spring, which holds loads indefinitely without consuming power or wearing out.

Adding a counterbalance is frequently the highest value change available to a mechanism that is marginal on torque.
