...
An arm is a lever, and gravity pulls on it at every point along its length. The torque required to hold it is not a fixed number: it depends on the angle, and it is largest exactly where teams tend not to check.
...
2. Apply a safety factor of at least 2. Static torque is only what holds the arm still; accelerating it, catching it after a bump, and resisting a defensive robot all demand more.
