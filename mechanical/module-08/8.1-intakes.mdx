...
The intake is where the robot meets the game, and it is where cycle time is usually won or lost. A scoring mechanism that is 0.5 seconds faster saves 0.5 seconds. An intake that fails one attempt in five costs a full retry cycle every fifth try, which is far more.

Reliability is the design target, not speed.
...
**Height relative to the element.** The intake must contact the element in a place that pulls it in rather than pushing it away. Contacting a round element above its center pushes it away; contacting below the center rolls it in. This single relationship explains most intake failures.
